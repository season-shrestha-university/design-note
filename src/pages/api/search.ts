...
export const GET: APIRoute = async (context) => {
  ...
  if (redis) {
    try {
      ...
      console.error(
        "Redis cache read error, falling back to live search:",
        err,
      );
    }
  }
  ...
  if (!ai) {
    console.warn(
      "GEMINI_API_KEY environment variable is not set. Cannot run AI search.",
    );
    return new Response(JSON.stringify([]), { status: 200 });
  }
  ...
  if (!supabase) {
    console.warn(
      "Supabase client is not configured. Cannot run database vector search.",
    );
    return new Response(JSON.stringify([]), { status: 200 });
  }
  ...
  try {
    ...
    const { data: matchedArticles, error: supabaseError } = await supabase.rpc(
      "match_articles",
      {
    ...
    if (redis) {
      try {
        ...
        console.log(
          `🔴 Cache MISS for query: "${normalizedQuery}" — cached for 24h`,
        );
      } catch (err) {
